...
#Spawn Chance
@export var enemy_spawn_change : float
#add other items here ei. healing, equip. etx.

@export var max_enemies_per_room : int
#add other spawn chances per room for desired items
...
func check_room(x : int, y : int, remaining : int, general_direction : Vector2, first_room : bool = false) -> void:
	...
	if east or first_room:
		check_room(x, y + 1, max_remaining if first_room else remaining -1, Vector2.RIGHT if first_room else general_direction)
	if west or first_room:
		check_room(x, y + 1, max_remaining if first_room else remaining -1, Vector2.LEFT if first_room else general_direction)
